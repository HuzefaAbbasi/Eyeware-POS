...
public class UpdateVendorController implements Initializable {
    ...
    private static VendorController vendorControllerInUpdateVendor;

    // This Method is setting the value of the static object created above
    public static void setVendorControllerInUpdateVendor(VendorController controller) {
        vendorControllerInUpdateVendor = controller;
    }
...
nameField.requestFocus();
    ...
    void onSearchClicked(ActionEvent event) throws IOException {
        ...
        if (searchField.getText().trim().equals("*")) {
            Function.fxmlFileName = "VendorModule/UpdateVendor.fxml";
            Function.title = "Update Vendor";
    ...
    void onUpdateClicked(ActionEvent event) {
        try {
            Connection connection = DriverManager.getConnection(Function.databasePath);
            boolean fieldsNotEmpty = true;boolean vendorSelected=true;
            ...
            if (fieldsNotEmpty) {
                if (Function.checkPhoneNo(phoneNumberField.getText().trim())) {
                    if (Function.checkAlphabets(nameField.getText().trim())) {
                            ...
                            String query = "Update Vendor set vendorPhoneNO = '"+phoneNumberField.getText().trim()+"' , vendorName = '"+nameField.getText().trim()+"' , " +
                                    "vendorAddress = '"+addressTextArea.getText().trim()+"' Where vendorId = "+vendorId;
                            Statement statement = connection.createStatement();
                            statement.execute(query);
                            Function.alert(Alert.AlertType.INFORMATION, "Congratulations", "Vendor Updated Successfully");
                            statement.close();
                            connection.close();
                            ...
                            if (vendorControllerInUpdateVendor != null) {
                            vendorControllerInUpdateVendor.refreshTableData();
                            }
                ...
                Function.alert(Alert.AlertType.ERROR, "Error", "Select the Vendor first");
                searchField.clear();
                searchField.requestFocus();
            }
